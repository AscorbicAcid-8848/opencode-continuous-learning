...
```text
第 1 层：安装与发现
scripts/install.sh、scripts/install.ps1、install/continuous-learning.ts
        ↓
第 2 层：OpenCode 运行编排
src/plugin.ts
        ↓
...
src/core.ts 中的 LearningStore 和配置/复盘函数
        ↓
第 4 层：磁盘数据
MEMORY.md、USER.md、项目 MEMORY.md、SKILL.md、JSON 状态
...
[`scripts/install.sh`](../scripts/install.sh) 和 [`scripts/install.ps1`](../scripts/install.ps1) 是安装程序。它们负责复制文件、备份旧版本、安装命令和注册 TUI。它们不判断复盘阈值，也不读写 Memory，所以不属于核心学习逻辑。

[`install/continuous-learning.ts`](../install/continuous-learning.ts) 是很薄的服务端桥接入口。安装后它位于 OpenCode 自动扫描的 `plugins/` 目录。它只做一件事：导入 `src/plugin.ts` 的默认导出，再原样导出给 OpenCode。
...
[`src/tui.ts`](../src/tui.ts) 是独立的界面控制器。它读取配置、生成选项、响应用户选择，再调用 `core.ts` 的配置函数保存。它不会直接创建 `LearningStore`，也不会直接改 Memory。
...
[`tests/core.test.ts`](../tests/core.test.ts)、[`tests/plugin.test.ts`](../tests/plugin.test.ts) 和 [`tests/tui.test.ts`](../tests/tui.test.ts) 是验证程序。它们会主动创建对象和调用方法，但只在执行测试时运行。
...
最外层是一个异步箭头函数。它不是类，也不是对象。OpenCode 调用它后，它才返回一个对象。返回的对象就是 Hooks 对象，里面的 `config`、`experimental.chat.system.transform` 和 `event` 是方法式回调，`tool` 属性则包含四个工具对象。
...
| `tool` | OpenCode/Agent | Agent 调用工具时 | 执行 Memory、Skill、状态和模式操作 |
| `experimental.chat.system.transform` | OpenCode | 组装模型系统提示时 | 注入持久学习快照 |
| `event` | OpenCode | 会话事件发生时 | 处理压缩、删除和空闲事件 |

`tool` 自己不是函数，而是一个对象；它的四个属性分别是 `tool(...)` 工厂创建的工具对象。

## 第六章：四个工具对象怎样调用 LearningStore
...
入口是 `learning_skill.execute(args, context)`。列表和查看调用 `store.listSkills()` 与 `store.viewSkill()`；创建和更新根据当前会话是否属于后台复盘，决定 `owner/origin` 是 `agent` 还是 `user`，然后调用 `store.createSkill()` 或 `store.updateSkill()`。
...
`FIELD_VIEWS` 是普通对象，键与 `LearningConfig` 的 14 个配置键一一对应。每个值也是对象，保存中文标题和分组。

`PanelAction` 是类型，不是对象。它规定用户选中的值只能是三种形状：修改字段、恢复默认或关闭面板。运行时真正的 `{ type: "field", key }` 才是对象。
...
`showResetConfirm(...)` 显示确认框，确认后调用 `resetConfig()`。恢复默认只改 14 个已知配置，不删除 Memory、Skill 和检查点。
...
第一遍只读 [`install/continuous-learning.ts`](../install/continuous-learning.ts)，确认服务端是怎样被发现的。
...
第四遍选择一条最短调用链，例如 `learning_memory view → store.readMemory()`，实际跟一次调用。

第五遍跟完整自动复盘链：`session.idle → maybeAutoReview → 子会话 → learning_* → LearningStore → checkpoint`。

第六遍读 [`src/tui.ts`](../src/tui.ts)，理解另一条独立入口怎样通过同一配置文件控制服务端。
...
[`install/continuous-learning.ts`](../install/continuous-learning.ts)：只负责让 OpenCode 找到服务端插件。

[`commands/*.md`](../commands/learn.md)：只负责指导前台 Agent 主动学习或复盘，真正写入仍经过工具和 `LearningStore`。

如果把整个系统想成一家公司：OpenCode 是客户，`plugin.ts` 是调度中心，`LearningStore` 是档案管理员，`core.ts` 的独立函数是档案规则和计算工具，`tui.ts` 是设置前台，Markdown/JSON 文件是档案柜。这就是本项目的层级、对象、方法和调用关系。
