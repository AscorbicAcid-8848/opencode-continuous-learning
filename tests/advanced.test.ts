...
import { ExternalMemoryAdapter } from "../src/external.ts";
import { LearningJourneyStore } from "../src/journey.ts";
import { applyPendingRecord, PendingWriteStore } from "../src/pending.ts";
import { SessionSearchStore } from "../src/search.ts";
import { DEFAULT_CONFIG } from "../src/config.ts";
import { type TranscriptItem } from "../src/review.ts";
...
const transcript = (topic: string): TranscriptItem[] => [
  ...
  },
  {
    ...
    toolCalls: [
      { name: "read", status: "completed", output: `${topic} details` },
    ],
  },
];
...
test("session search indexes, discovers, reads, scrolls, browses, and removes sessions", async () => {
  ...
  try {
    ...
    const browsed = search.browse(10) as {
    ...
    assert.equal(
      search.isExternalTurnSynced("mem0", "session-alpha", "a1"),
      false,
    );
    ...
    assert.equal(
      search.isExternalTurnSynced("honcho", "session-alpha", "a1"),
      false,
    );
    ...
    assert.equal(
      reopened.isExternalTurnSynced("mem0", "session-alpha", "a1"),
      true,
    );
...
test("pending writes survive on disk and move to approval or rejection history", async () => {
  ...
  try {
    ...
    const applied = await pending.approve(
      first.id,
      async (record) => record.payload.kind,
    );
    ...
    assert.match(
      await readFile(
        join(pending.historyRoot, `${first.id}.approved.json`),
        "utf8",
      ),
      /"status": "approved"/u,
    );
    assert.match(
      await readFile(
        join(pending.historyRoot, `${second.id}.rejected.json`),
        "utf8",
      ),
      /"status": "rejected"/u,
    );
...
test("journey graph links learned memories and Skills and Skill deletion is recoverable", async () => {
  ...
  try {
    ...
    const deleted = await store.deleteSkill({
      name: "token-refresh",
      origin: "agent",
    });
    ...
    assert.match(
      await readFile(
        join(deleted.archivePath, ".continuous-learning-archive.json"),
        "utf8",
      ),
...
test("Mem0 adapter uses official search/add shapes without exposing credentials", async () => {
  ...
  try {
    ...
    assert.equal(
      JSON.stringify(adapter.status()).includes("test-secret-key"),
      false,
    );
  } finally {
