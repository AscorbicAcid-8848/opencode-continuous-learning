...
test("TUI panel registers a slash command and edits, validates, and resets settings", async () => {
  ...
  try {
    ...
    const resourceFields = options.filter((option) => option.category === "资源限制")
    assert.equal(resourceFields.length, 5)
    for (const option of resourceFields) {
      assert.match(String(option.footer), /^\[[^\[\]]+\]$/u)
    }
    const enabled = options.find((option) => (option.value as UnknownRecord).key === "enabled")
    assert.ok(enabled)
    ;(first.onSelect as (option: UnknownRecord) => void)(enabled)
    await waitFor(async () => {
      const value = JSON.parse(await readFile(configPath, "utf8")) as { enabled: boolean }
    ...
    assert.equal((JSON.parse(await readFile(configPath, "utf8")) as { memoryEveryTurns: number }).memoryEveryTurns, 10)
    assert.equal(toasts.at(-1)?.variant, "error")
    ;(prompt.onConfirm as (value: string) => void)("20")
    await waitFor(async () => {
      const value = JSON.parse(await readFile(configPath, "utf8")) as { memoryEveryTurns: number }
    ...
    await waitFor(async () => {
      ...
      return value.enabled === true && value.memoryEveryTurns === DEFAULT_CONFIG.memoryEveryTurns
    })
